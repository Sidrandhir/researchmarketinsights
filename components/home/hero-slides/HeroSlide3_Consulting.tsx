...
const HeroSlide3_Consulting: React.FC<Props> = ({ isActive, t, currentLang }) => {
    ...
    <div onMouseDown={(e) => e.stopPropagation()}>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className="text-center max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="inline-flex items-center gap-2 bg-black/20 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm font-medium mb-8">
          ...
          <span className="text-shadow">{t('home.hero.slide3.badge')}</span>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="heading-responsive font-bold text-white mb-6 leading-tight text-shadow-md">
          <span className="block">{t('home.hero.slide3.title')}</span>
        </motion.h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <Link href={`/${currentLang}/sample-request`} className="btn-primary bg-white text-primary-600 hover:bg-gray-100 py-4 px-8 text-lg rounded-xl shadow-lg">
            {t('home.hero.slide3.cta1')}
          </Link>
          <Link href={`/${currentLang}/contact`} className="text-white font-medium hover:underline inline-flex items-center gap-2 py-4 px-8 text-shadow">
            {t('home.hero.slide3.cta2')}
...
export default HeroSlide3_Consulting;